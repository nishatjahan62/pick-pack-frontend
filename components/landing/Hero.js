  ...
  return (
    ...
    >
      <motion.div
        ...
        className="relative bg-white/90 dark:bg-white/10 backdrop-blur-xl border border-gray-200/80 dark:border-white/20 rounded-2xl px-5 py-4 text-gray-900 dark:text-white shadow-xl shadow-black/5 dark:shadow-black/30 cursor-default group overflow-hidden min-w-[155px]"
      >
        {/* Hover glow */}
        <div className="absolute inset-0 rounded-2xl bg-gradient-to-br from-black/5 dark:from-white/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
        ...
        <div className="relative z-10 flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-gray-500 dark:text-gray-400 text-xs mt-1">{card.label}</div>
          </div>
        </div>
...
export default function Hero() {
  return (
    <section className="relative h-[calc(100vh-57px)] flex items-center justify-center overflow-hidden transition-colors">

      {/* Background */}
      <div
        className="absolute inset-0 bg-cover bg-center bg-no-repeat opacity-100 dark:opacity-100"
        style={{ backgroundImage: "url('/hero.jpg')" }}
      />

      {/* Overlay - light in light mode, dark in dark mode */}
      <div className="absolute inset-0 dark:bg-gradient-to-b from-black/80 via-black/70 to-black/90" />
      ...
      >
        <div className="max-w-2xl w-full">
          ...
          <motion.h1
            variants={fadeUp}
            className="text-4xl sm:text-5xl md:text-6xl font-extrabold text-gray-900 dark:text-white leading-tight mb-4 tracking-tight"
          >
            Manage your{' '}
            <span className="relative inline-block">
              <span className="text-green-600 dark:text-green-400">stock</span>
              <motion.span
                className="absolute -bottom-1 left-0 h-[3px] bg-green-600 dark:bg-green-400 rounded-full"
                initial={{ width: 0 }}
                animate={{ width: '100%' }}
          ...
          <motion.div variants={fadeUp} className="grid grid-cols-2 gap-3 max-w-md mx-auto w-full">
            ...
            >
              <Link
                href="/signup"
                className="relative overflow-hidden flex items-center justify-center gap-2 bg-green-600 hover:bg-green-500 dark:bg-green-500 dark:hover:bg-green-400 text-white font-semibold px-5 py-3.5 rounded-xl text-sm transition-all duration-300 hover:scale-105 group w-full shadow-md"
              >
            ...
            {/* Button 2 — Demo */}
            <motion.div
              animate={{ borderColor: ['rgba(0,0,0,0.15)', 'rgba(0,0,0,0.4)', 'rgba(0,0,0,0.15)'] }}
              transition={{ duration: 2.5, repeat: Infinity, ease: 'easeInOut' }}
              className="rounded-xl border border-gray-300 dark:border-white/20"
            >
              <Link
                href="/login"
          ...
          >
            {stats.map((stat, i) => (
              <div key={i} className="flex items-center gap-6 sm:gap-10">
                {i !== 0 && <div className="w-px h-8 bg-gray-300 dark:bg-white/20 hidden sm:block" />}
                <motion.div
                ...
                >
                  ...
                  <div className="text-xs text-gray-500 dark:text-gray-400 mt-0.5 group-hover:text-gray-800 dark:group-hover:text-gray-200 transition-colors duration-300">
                    {stat.label}
                  </div>
                </motion.div>
              </div>
